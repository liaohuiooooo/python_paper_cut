...
# 根据长向量找出顶点
def extractPeek(array_vals, min_vals=100, min_rect=200):
    extrackPoints = []
    startPoint = None
    endPoint = None
...
# 寻找边缘，返回边框的左上角和右下角（利用cv2.findContours）找所有边界
def findBorderContours(path, maxArea=50):
    ...
    for contour in contours:
        # 将边缘拟合成一个边框
        x, y, w, h = cv2.boundingRect(contour)  # 返回边框的左上角和右下角
        if w * h > maxArea:
            border = [(x, y), (x + w, y + h)]
            borders.append(border)
    return borders
...
def showResults(path, borders, results=None):
    ...
    for i, border in enumerate(borders):
        cv2.rectangle(img, border[0], border[1], (0, 0, 255))  # 画出矩行;  矩形边框（Bounding Rectangle）
        if results:
...
def predict(modelpath, imgData):
    ...
    img = imgData.astype('float32') / 255  # numpy astype函数用于array中数值类型转换
